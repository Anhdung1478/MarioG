...
QuestionBlock::QuestionBlock(sf::Vector2f pos) 
    : numberOfCoins(1), Block(pos, sf::Vector2f(16, 16), "question-block") 
{
    InitSpritesSheet();
    ...
    coins_animation->setVelocity(sf::Vector2f(0.f, -2.f));
    ...
    : numberOfCoins(1), Block(pos, size, name) 
{
    ...
    coins_animation->addAnimationStep("coin[8]");
    coins_animation->setPosition(sf::Vector2f(p_body->getPosition().x, p_body->getPosition().y - p_body->getSize().y));
    coins_animation->setVelocity(sf::Vector2f(0.f, -10.f));
    coins_animation->setLoop(false);
    coins_animation->setAnimationState(false);
}
